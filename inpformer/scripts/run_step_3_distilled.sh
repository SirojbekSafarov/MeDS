        ...
        for ratio in "${noise_ratios[@]}"; do
            for seed in "${seeds[@]}"; do
                for mad_factor in "${mad_factors[@]}"; do
                    ...
                    SAVE_DIR="/path/to/experiments/inpformer/mvtec/stage_3_data_selection/distilled/noise_ratio_${ratio}/seed_${seed}/z${mad_factor}/ensemble_${ensemble}_p${scaled}/${loss_function}"
                    DISTILL_OUT_DIR="/path/to/experiments/inpformer/mvtec/stage_2_distillation/noise_ratio_${ratio}/seed_${seed}/ensemble_${ensemble}_p${scaled}/${loss_function}"
