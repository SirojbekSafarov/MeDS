...
def get_data_transforms(size, isize, mean_train=None, std_train=None):
    ...
    gt_transforms_mask = transforms.Compose([
        transforms.Resize((size, size)),
        transforms.CenterCrop(isize),
        transforms.Resize((28, 28)),
        transforms.ToTensor()])
    return data_transforms, gt_transforms, gt_transforms_mask
...
class RealIADDataset_fuad(torch.utils.data.Dataset):
    def __init__(self, root, category, transform, gt_transform, phase, type='realiad_jsons_fuiad_0.0'):
        self.img_path = os.path.join(root, 'realiad_1024', category)
        self.transform = transform
        self.gt_transform = gt_transform
        self.phase = phase
        ...
        # json_path = os.path.join(root, 'realiad_jsons', 'realiad_jsons', category + '.json')
        json_path = os.path.join(root, 'realiad_jsons', type, category + '.json')
        with open(json_path) as file:
            class_json = file.read()
        class_json = json.loads(class_json)
...
class RealIADDataset_fuad_debug(torch.utils.data.Dataset):
    def __init__(self, root, category, transform, gt_transform, phase, type='realiad_jsons_fuiad_0.0'):
        self.img_path = os.path.join(root, 'realiad_1024', category)
        self.transform = transform
        self.gt_transform = gt_transform
    ...
    def __getitem__(self, idx):
        ...
        return img_path, gt, label


class RealIADDataset_fuad_mem_recom_distill(torch.utils.data.Dataset):
    def __init__(self, root, category, transform, gt_transform, phase, type='realiad_jsons_fuiad_0.0', memory_dir='memory_anomaly_maps_ensemble_100_fuad_0.0'):
        self.img_path = os.path.join(root, 'realiad_1024', category)
        self.transform = transform
        self.gt_transform = gt_transform
        self.phase = phase
        ...
        self.label_dir = os.path.join(root,'realiad_1024', category, memory_dir)
    ...
    def __getitem__(self, idx):
        ...
        label_path = os.path.join(self.label_dir, file_name + '.pth')
