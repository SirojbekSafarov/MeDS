...
class ViTill(nn.Module):
    ...
    def forward_feature(self, x):
        ...
        for i, blk in enumerate(self.encoder.blocks):
            ...
            if i in self.target_layers:
                en_list.append(x)
        feature = self.encoder.norm(feature)
        feature = feature[:, 1 + self.encoder.num_register_tokens:, :]
        side = int(math.sqrt(en_list[0].shape[1] - 1 - self.encoder.num_register_tokens))
        ...
        # loss = self.criterion(en, de, p=p, factor=0.1)

        return en, de, feature
    ...
    def forward_inference(self, x):
        ...
        with torch.no_grad():
            ...
            # Encode
            # If you do not need intermediate encoder layers at inference,
            # process up to the needed depth and do not store intermediate results
            for i, blk in enumerate(self.encoder.blocks):
                if i <= self.target_layers[-1]:
                    x = blk(x)
                else:
            ...
            # Compute spatial dimension (assuming the same logic for output shape)
            side = int(math.sqrt(x.shape[1] - 1 - self.encoder.num_register_tokens))

            # If class token is not needed at inference output, remove it
            if self.remove_class_token:
                x = x[:, 1 + self.encoder.num_register_tokens:, :]

            # If needed, fuse features (if your final output depends on fused features)
            # If not strictly necessary at inference, skip or simplify
            x = self.fuse_feature([x])

            # Bottleneck layers
            for blk in self.bottleneck:
                x = blk(x)

            # If mask is not needed at inference, skip this
            attn_mask = self.generate_mask(side, x.device) if self.mask_neighbor_size > 0 else None

            # Decode
            # If intermediate decoding layers aren't needed, just run them through
            for blk in self.decoder:
                x = blk(x, attn_mask=attn_mask)

            # If at inference you only need the final output (e.g., a segmentation map),
            # you might directly process 'x' into the final output form here.
            # The following code assumes you need a final feature map like in the original code:

            if not self.remove_class_token:
                # Remove class token if not needed
                x = x[:, 1 + self.encoder.num_register_tokens:, :]
