...
def cosine_data_sample(model, distilled_model_anomaly_maps, data_list, save_dir, iter, total_iters, mad_factor=3, beta_end=0.5):
    subsets = []
...
warnings.filterwarnings("ignore")
def train(item_list, output_dir, distill_output_dir, total_iters=10000, batch_size=16, encoder_name='dinov2reg_vit_base_14', distill_initialized=True, mad_factor=1, beta_end=0.5):
    setup_seed(1)
    ...
    if distill_initialized:
        model.load_state_dict(distilled_model_state_dict)
    else:
    ...
    while True:
        ...
        train_data_list_sub = cosine_data_sample(model=model, distilled_model_anomaly_maps=distilled_model_anomaly_maps, data_list=train_data_list, save_dir=args.output_dir, iter=it, total_iters=total_iters, mad_factor=mad_factor, beta_end=beta_end)
...
if __name__ == '__main__':
    ...
    print_fn(device)
    print_fn(f'distill_initialized = {args.distill_initialized}')
    ...
    train(item_list, args.output_dir, args.distill_output_dir, total_iters=args.n_iters, batch_size=args.batch_size, encoder_name=args.encoder, distill_initialized=args.distill_initialized, mad_factor=args.mad_factor, beta_end=args.beta_end)
